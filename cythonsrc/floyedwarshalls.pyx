    ...
    for k in range(n):
        ...
        for i in range(k):
            ...
            if dist_ik != np.inf and i != k:
                for j in range(i):
                    dist_matrix[i, j] = min(dist_matrix[i, j], dist_matrix[i, k] + dist_matrix[j, k])
                    next_matrix[i, j] = next_matrix[i,k]
        for i in range(k, n):
            dist_ik = dist_matrix[i, k]
            if dist_ik != np.inf and i != k:
                for j in range(k):
                    dist_matrix[i, j] = min(dist_matrix[i, j], dist_matrix[k, i] + dist_matrix[j, k])
                    next_matrix[i, j] = next_matrix[k, i]
                for j in range(k, i):
                    dist_matrix[i, j] = min(dist_matrix[i, j], dist_matrix[k, i] + dist_matrix[k, j])
                    next_matrix[i, j] = next_matrix[k,i]
